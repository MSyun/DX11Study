...
namespace MSLib {
	...
	HRESULT DX11Base::CreateRenderTargetView() {
		...
		hr = IResource::GetDevice()->CreateRenderTargetView(pBackBuffer, nullptr, &m_pRenderTargetView);
	...
	HRESULT DX11Base::CreateDepthStencilView() {
		...
		hr = device->CreateDepthStencilView(m_pDepthStencil, nullptr, &m_pDepthStencilView);
