...
namespace MSLib {
	...
	//				セーブ				//
	//									*/
	int SaveManagerBase::Save(const std::string& filename) {
		// セーブファイルオープン
		m_fs.open(filename.c_str(), std::ios_base::out | std::ios_base::binary);
		if (!m_fs.is_open())
			return 0;
	...
	//				ロード				//
	//									*/
	int SaveManagerBase::Load(const std::string& filename) {
		// セーブファイルオープン
		m_fs.open(filename.c_str(), std::ios_base::in | std::ios_base::binary);
		if (!m_fs.is_open())
			return NO_SAVEFILE;
