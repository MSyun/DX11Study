...
namespace MSLib {
	namespace Sound {
			...
			IXAudio2* pXaudio2)
		{
			...
			Sound->pDataAudio = nullptr;
			Sound->pSource = nullptr;
			...
			m_mapSound.insert(std::pair< std::string, _tSound* >(name, Sound));
		...
		//				削除				//
		//									*/
		void SoundBase::Delete(const std::string name) {
			//----- 初期化
		...
		//				再生				//
		//									*/
		HRESULT SoundBase::Play(const std::string name, int loop) {
			//----- 登録済み確認
			auto it = m_mapSound.find(name);
		...
		//				停止				//
		//									*/
		void SoundBase::Stop(const std::string name) {
			//----- 登録済み確認
			auto it = m_mapSound.find(name);
